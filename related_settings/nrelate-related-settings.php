...
?>
	...
	<div class="wrap" style="margin: 10px 0 0 0;">
        ...
        <button type="button" class="button-primary" onClick="return nrelate_popup_preview('<?= NRELATE_RELATED_SETTINGS_URL ?>','<?=$wp_root_nr?>','<?=NRELATE_RELATED_PLUGIN_VERSION ?>');"> <?php _e('Preview','nrelate'); ?> </button>
		<?php settings_fields('nrelate_related_options'); ?>
		<?php do_settings_sections(__FILE__);?>
		<br><button type="button" class="button-primary" onClick="return nrelate_popup_preview('<?= NRELATE_RELATED_SETTINGS_URL ?>','<?=$wp_root_nr?>','<?=NRELATE_RELATED_PLUGIN_VERSION ?>');"> <?php _e('Preview','nrelate'); ?> </button>
		<p class="submit">
			<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','nrelate'); ?>" />
		</p>
		</form>
